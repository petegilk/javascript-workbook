...
const stringsToNumbs = strNums.map(function(item){
...
sumEvens(stringsToNumbs);

// Find the index of the first value when added to it's index = 512 (#ATX!!)
const atxIdx = stringsToNumbs.filter((num, i, arr) => {
...
// I tried to get this to work with the line 34 below,
//  but had no luck. Logged within callback instead.
...
let uniqueWeather = [];
const weatherStates = weather.forEach((item, i, arr) => {
  
  if (!uniqueWeather.includes(item.weather_state_name)) {
    uniqueWeather.push(item.weather_state_name);
  }
});
console.log(uniqueWeather)

//find the id of the object in weather that has a min_temp of 15.915

const idealTemp = weather.forEach((item, i, arr) => {
  if (item.min_temp === 15.915) {
    console.log(`item with min temp of 15.915 is ${item.id}`);
    return item.id;
  }
});
// This console log is returning undefined, also logged within forEach
